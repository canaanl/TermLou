...
class RootfsPatcher {
    ...
    fun ensureRootfsDns(
        ...
        val resolvTmp = File(resolvConf.parentFile, "${resolvConf.name}.tmp")
        resolvTmp.writeText(
            buildString {
    ...
    fun ensureGroupNames(etcDir: File, groupIds: List<Long>) {
        ...
        val additions = groupIds
            ...
            .map { id ->
                ...
                var name = baseName
                var n = 0
                while (name in existingNames || name in takenNames) {
                    n++
                    name = if (n == 1) "${baseName}_workspace" else "${baseName}_workspace$n"
                }
                takenNames += name
                "$name:x:$id:"
            }
