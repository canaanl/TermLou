...
| 版本 | versionCode | 内容 |
|------|------------|------|
| **5.4.2** | 5402 | termlou-ui 浮窗「不关窗连续换屏」错位修复（治本：换屏同步归属编号）：① 按钮栏同构复用不重建 view，其 click 仍闭包上一屏回调 → 点新屏按钮把结果写给上一屏等待方、本屏永远收不到，改为触发时读当前屏回调（Renderer.currentOnResult），同构但换主题/主色也重建刷新配色；② 返回键/点外部/关窗判断仍拿第一屏的请求与回调 → Overlay 增加 currentOnResult，updateContent 换屏即同步，shouldDismiss 也改按当前屏按钮表；③ 新请求原位接管时旧请求若仍在等待会挂到超时 → OverlayBridge 换屏先停旧超时器，再给被顶掉的等待方写明确 dismiss；新增 EntryOwnership 归属登记（同一请求只结算一次，陈旧点击被闸门拦下，不再覆盖 res、不再惊动别人的超时器）；新增 8 单测（282 全绿） |
| **5.4.1** | 5401 | PTY JNI 层问题报告 5 项落地修复：native 签名对齐上游 9 参/5 参（createSubprocess/setPtyWindowSize，ws_xpixel/ws_ypixel 按单格×行列生效，PtyJni 单格默认 8×16 且 Method 只解析一次不再每次 getDeclaredMethod）；删除 4 个无声明、无调用方的死导出（readPty/writePty/read/write，无边界校验的不可达代码）；waitFor/waitPid 补 EINTR 重试（信号不再被误当退出状态）；tun2socks 改 execve + allowlist 最小环境（不再继承 ART 全量 environ）；会话拆除统一 SessionTerminator（主终端退出也杀整棵进程树、LAN 杀后后台收割防僵尸，修 setSession(null) 先于 destroy 导致的空转）；新增 PTY JNI 真机冒烟 androidTest（建会话→回显→resize/SIGWINCH→杀树→收尸，connectedDebugAndroidTest 运行）；274 单测全绿（app 223 + workspace 51） |
| **5.4.0** | 5400 | 五个 Tab 说明页全面改版为**说明书体**：24 节按「用途 → ▸ 操作（手势→效果）→ ● 规则 → ⚠ 注意」四段式重排，清单型内容一行一项、步骤/操作一行一动作；笔记页新增「搜索」节（笔记/待办/标签三框同款、全字段、多词 AND、同音容错、拼音全拼/首字母、返回先清词、200ms 防抖）；条目行悬挂缩进渲染（符号下折行对齐文字）；手势逐条对照代码核实（文件点按=弹「导出/删除」菜单、长按文件夹同款、流量日志长按=阻断菜单、笔记长按=重命名、列表顶部「返回」行、流量日志搜索框等），不虚构操作；中英两版结构逐行对应（键名不变，TabIntroContent 仅 +1 节）；223 单测全绿 |
| **5.3.0** | 5300 | 笔记搜索架构收口（写时建索引、查询期零 .txt 读盘）：正文小写镜像＋拼音主/副读音＋首字母剖面＋ bigram 表随保存/入库物化进库，schema 升 v2（版本不符删库走 txt 恢复通道，删库恢复照旧）；查询三段式——标题/标签/待办内存精确打分 ∪ 正文库召回（≥3 字 trigram 短语、2 字 bigram、1 字 LIKE、汉字读音组合双剖面、2~16 字 bigram OR 容错、拉丁补拼音/首字母列），同词分支库内 UNION、跨词库内 INTERSECT 只回传交集，候选批量取正文后由原精排内核精确过滤（标题＞标签＞正文＞容错＞拼音＞首字母，排序语义零改动，新旧对拍测试锁定）；入库跳过无索引列 DELETE 修 O(N²)（万条 187s→2.7s）；列表快照缓存（写路径失效）；万条基准锁定：典型查询单次 <50ms（实测 1~33ms）、全库级最坏路径单设 500ms 档；新增 6 单测（223 全绿） |
| **5.2.1** | 5201 | 笔记列表搜索升级为**全字段**：标题＋标签（标题档）＋正文＋所属待办文本（正文档）一起搜——搜标签名或待办词也能把归属笔记捞出来，多词跨字段互补（词1 在标题、词2 在标签也算命中）；todos 一次全量查出内存分组防 N+1；新增 3 个单测（217 全绿） |
| **5.2.0** | 5200 | 笔记/待办/标签三框统一模糊搜索：笔记列表（主界面 Tab＋独立笔记页）新增同款搜索框（200ms 防抖、空结果提示、返回键先清词）；空格多词分词（笔记/待办 AND、标签 OR）；错别字同音容错（「苹菓→苹果」「超屎→超市」，差异字必须同音防噪声，6 字词容多字/少字）；内置拼音表（Unicode Unihan 26711 字，无声调、多音字全收、APK 资源不进数据目录）支持全拼/首字母/同音字搜索（`pingguo`、`pg`、搜「香蕉」命中「相交」）；相关度排序（标题＞正文＞容错＞拼音＞首字母）；数据零影响、db 结构不变、删库从 txt 恢复照常；新增 15 个单测（214 全绿） |
| **5.1.9** | 5109 | 全量 bug 修复（16 项）：崩溃浮窗真正等显示完再杀进程（此前必被抢先、通知也不发），崩溃通知改 ID5 不再顶掉 VPN 通知；250ms 内连点切 Tab 错位；LAN 共享双启动竞态＋启动失败误清凭据；网盘上传改分块扫描（不再把 64MB 整读进堆）＋符号链接目录可传；WS 会话关闭杀整棵进程树（不留孤儿 shell）；流量看板 300ms 节流丢尾补发＋淘汰条目修正计数；预热前台服务/wakelock 泄漏；待办勾选改数据先行（动画中断不丢状态）；终端真彩色 SGR 38/48 子参数正确解析；笔记库连接随页/随主界面关闭；`\c` 小写化避开土耳其 I；快捷键保存越界保护；新增 15 个单测（199 全绿） |
| **5.1.8** | 5108 | 待办看板：条目下不再显示归属笔记名；事项字号 15 收至 14 |
